...
ptm <- proc.time()
small <- properties(nsim=100,n=1000,m=20,maxit=1, ntrees=100, randomsel=3,
        B=c(beta0=0, beta1=.34, beta2=.34, beta3=.34, beta4=.34, beta5=.34, beta6=.11))
proc.time() - ptm
small

ptm <- proc.time()
medium <- properties(nsim=100,n=1000,m=20,maxit=1, ntrees=100, randomsel=3,
        B=c(beta0=0, beta1=.31, beta2=.31, beta3=.31, beta4=.31, beta5=.31, beta6=.28))
proc.time() - ptm
medium

ptm <- proc.time()
large <- properties(nsim=100,n=1000,m=20,maxit=1, ntrees=100, randomsel=3,
        B=c(beta0=0, beta1=.28, beta2=.28, beta3=.28, beta4=.28, beta5=.28, beta6=.42))
proc.time() - ptm
large

test <- createdata(n=100000, B=c(beta0=0, beta1=.34, beta2=.34, beta3=.34, beta4=.34, beta5=.34, beta6=.11))    # Total: .50
rsquare1 <- summary(lm(y~x1+x2+x3+x8+x9, data=test))$r.squared
rsquare2 <- summary(lm(y~x1+x2+x3+x8+x9+I(x3^2), data=test))$r.squared
rsquare2
rsquare2-rsquare1
((rsquare2-rsquare1)/(1-rsquare2))

test <- createdata(n=100000, B=c(beta0=0, beta1=.31, beta2=.31, beta3=.31, beta4=.31, beta5=.31, beta6=.28))    # Total: .50
rsquare1 <- summary(lm(y~x1+x2+x3+x8+x9, data=test))$r.squared
rsquare2 <- summary(lm(y~x1+x2+x3+x8+x9+I(x3^2), data=test))$r.squared
rsquare2
rsquare2-rsquare1
...
test <- createdata(n=100000, B=c(beta0=0, beta1=.28, beta2=.28, beta3=.28, beta4=.28, beta5=.28, beta6=.42))    # Total: .50
rsquare1 <- summary(lm(y~x1+x2+x3+x8+x9, data=test))$r.squared
rsquare2 <- summary(lm(y~x1+x2+x3+x8+x9+I(x3^2), data=test))$r.squared
rsquare2
rsquare2-rsquare1
...
createdata <- function(n=1000, B=c(beta0=0, beta1=.5, beta2=.5, beta3=.5, beta4=.5, beta5=.5, beta6=.5))
{
  ...
  X <- matrix(c(Intercept, x[,1], x[,2] , x[,3], x[,8], x[,9], (x[,3]^2)),nrow=n)
...
test.impute <- function(data, m=20, method="rtree", maxit=1, ntrees=ntrees, randomsel=randomsel)
{
  ...
  fit <- with(imp, lm(y~x1+x2+x3+x8+x9+I(x3^2)))
  est <- pool(fit)
  tab <- summary(est)
...
maketable <- function(s, B=B)
{
  ...
  rownames(prop_table) <- names(coef(lm(y~x1+x2+x3+x8+x9+I(x3^2), data=createdata(n=100))))
...
small <- properties(nsim=50,n=1000,m=10,maxit=1, ntrees=50, randomsel=3,
        B=c(beta0=0, beta1=.19, beta2=.19, beta3=.19, beta4=.19, beta5=.19, beta6=.09))
proc.time() - ptm
small

ptm <- proc.time()
medium <- properties(nsim=50,n=1000,m=10,maxit=1, ntrees=50, randomsel=3,
       B=c(beta0=0, beta1=.16, beta2=.16, beta3=.16, beta4=.16, beta5=.16, beta6=.24))
proc.time() - ptm
medium

ptm <- proc.time()
large <- properties(nsim=50,n=1000,m=10,maxit=1, ntrees=50, randomsel=3,
        B=c(beta0=0, beta1=.10, beta2=.10, beta3=.10, beta4=.10, beta5=.10, beta6=.35))
proc.time() - ptm
large