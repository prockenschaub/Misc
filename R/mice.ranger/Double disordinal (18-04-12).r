summary(glm(y~x1+x2+x3+x4+x8+x9+x3*x4, data=testdata, family='binomial'))

### Base rate=0.5, effect size=low (exp(B)=1.5)
### Interactie: exp(b34)=1.5, b34=0.4 
testdata <- createdata(n=1000000, B=c(Intercept=0, x1B=.4, x2B=.5, x3B=-.2, x4B=-.2, x8B=.4, x9B=-1.1, x3Bx4B=.4))
sum(testdata[,1]==1)

### Base rate=0.5, effect size=high (exp(B)=3)
### Interactie: exp(b34)=3, b34=1.1 
testdata <- createdata(n=1000000, B=c(Intercept=0, x1B=.4, x2B=.4, x3B=-.2, x4B=-.2, x8B=.4, x9B=-1.3, x3Bx4B=1.1))
sum(testdata[,1]==1)
 
### Interactie: exp(b34)=1.5, b34=0.4
ptm <- proc.time()
dis_dis_small <- properties(nsim=100, n=1000, m=20, maxit=1, ntrees=100, randomsel=3,
  B=c(Intercept=0, x1B=.4, x2B=.5, x3B=-.2, x4B=-.2, x8B=.4, x9B=-1.1, x3Bx4B=.4))
proc.time() - ptm
dis_dis_small

### Interactie: exp(b34)=3, b34=1.1
ptm <- proc.time()
dis_dis_large <- properties(nsim=100, n=1000, m=20, maxit=1, ntrees=100, randomsel=3,
  B=c(Intercept=0, x1B=.4, x2B=.4, x3B=-.2, x4B=-.2, x8B=.4, x9B=-1.3, x3Bx4B=1.1))
proc.time() - ptm
dis_dis_large 
  
### Create binomial data
createdata <- function(n=1000, B=c(Intercept=0, x1B=-2.5, x2B=3, x3B=2, x4B=1.5, x8B=-1, x9B=-1.5, x3Bx4B=-5))
{
  library(MASS)
  ...
  X  <- model.matrix(~x1+x2+x3+x4+x8+x9+x3*x4)                               # Create data frame with dummys in order to perform matrix multiplication with the parameters (B)
...
test.impute <- function(data, m=20, method=method, maxit=1, ntrees=ntrees, randomsel=randomsel)
{
  ...
  fit <- with(imp, glm(y~x1+x2+x3+x4+x8+x9+x3*x4, family='binomial'))
  est <- pool(fit)
  tab <- summary(est)
...
maketable <- function(s, B=B)
{
  ...
  rownames(prop_table) <- names(coef(glm(y~x1+x2+x3+x4+x8+x9+x3*x4, data=createdata(n=100), family='binomial')))
