library(MASS)
summary(glm(y~x1+x2+x3+x4+x8+x9+x8*x9, data=testdata, family='binomial'))
### Base rate=0.5, effect size=low (exp(B)=1.5)
### Interactie: exp(b89)=1.5, b89=0.4 
testdata <- createdata(n=100000,  B=c(Intercept=0, x1B=.7, x2B=.4, x3B=-1.1, x4B=-.7, x8B=.7, x9B=-.2, x8Bx9B=.4))
sum(testdata[,1]==1)

### Base rate=0.5, effect size=high (exp(B)=3)
### Interactie: exp(b89)=3, b89=1.1 
testdata <- createdata(n=100000,  B=c(Intercept=0, x1B=.7, x2B=.5, x3B=-1.1, x4B=-1.1, x8B=.7, x9B=-.2, x8Bx9B=1.1))
sum(testdata[,1]==1)

### Interactie: exp(b89)=1.5, b89=0.4
ptm <- proc.time()
dis_or_small <- properties(nsim=200, n=1000, m=20, maxit=1, ntrees=100, randomsel=3,
  B=c(Intercept=0, x1B=.7, x2B=.4, x3B=-1.1, x4B=-.7, x8B=.7, x9B=-.2, x8Bx9B=.4))
proc.time() - ptm
dis_or_small

### Interactie: exp(b89)=3, b89=1.1
ptm <- proc.time()
dis_or_large <- properties(nsim=200, n=1000, m=20, maxit=1, ntrees=100, randomsel=3,
  B=c(Intercept=0, x1B=.7, x2B=.5, x3B=-1.1, x4B=-1.1, x8B=.7, x9B=-.2, x8Bx9B=1.1))
proc.time() - ptm
dis_or_large

### Create binomial data
createdata <- function(n=1000, B=c(Intercept=0, x1B=-2.5, x2B=3, x3B=2, x4B=1.5, x8B=-1, x9B=-1.5, x8Bx9B=-3))
{
  ...
  X  <- model.matrix(~x1+x2+x3+x4+x8+x9+x8*x9)                               # Create data frame with dummys in order to perform matrix multiplication with the parameters (B)
...
### Create missings
#makeMCAR <- function(data)
#{
#  r.mcar <- rbinom(nrow(data), 1, prob=.3)                                      # Random generation of missingness. prob=.3 is percentage missing
#  data[r.mcar==1,1] <- NA                                                       # Create missing values in dependent variable according to r.mcar
#  return(data)                                                                  # Returns data with missing values
#}
...
test.impute <- function(data, m=20, method=method, maxit=1, ntrees=ntrees, randomsel=randomsel)
{
  ...
  fit <- with(imp, glm(y~x1+x2+x3+x4+x8+x9+x8*x9, family='binomial'))
  est <- pool(fit)
  tab <- summary(est)
...
maketable <- function(s, B=B)
{
  ...
  rownames(prop_table) <- names(coef(glm(y~x1+x2+x3+x4+x8+x9+x8*x9, data=createdata(n=100), family='binomial')))
