...
testdata <- createdata(n=1000000, B=c(Intercept=0, x1B=.5, x2B=1, x3B=-1.1, x4B=-.5, x8B=.4, x9B=-.5, x1Bx2B=.4))
sum(testdata[,1]==1)

### Base rate=0.5, effect size=high (exp(B)=3)
### Interactie: exp(b12)=3, b12=1.1 
testdata <- createdata(n=1000000, B=c(Intercept=0, x1B=.5, x2B=1, x3B=-1, x4B=-.5, x8B=.5, x9B=-1, x1Bx2B=1.1))
sum(testdata[,1]==1)
